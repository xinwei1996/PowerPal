...
class WPDA
{
     ...
     */
    const  OPTION_WPDA_VERSION = array( 'wpda_version', '5.2.6' ) ;
    /**
     * Option wpda_setup_error and it's default value
     */
    const  OPTION_WPDA_SETUP_ERROR = array( 'wpda_setup_error', '' ) ;
    // Values: off = do not display setup errors, other = show.
    /**
     * Option wpda_show_whats_new and it's default value
     */
    const  OPTION_WPDA_SHOW_WHATS_NEW = array( 'wpda_show_whats_new', 'off' ) ;
    // Values: off = hide link to what's new page, other = show.
    /**
     * Option wpda_uninstall_tables and it's default value
     */
    const  OPTION_WPDA_UNINSTALL_TABLES = array( 'wpda_uninstall_tables', 'off' ) ;
    // On uninstall drop WPDA tables.
    /**
     * Option wpda_uninstall_options and it's default value
     */
    const  OPTION_WPDA_UNINSTALL_OPTIONS = array( 'wpda_uninstall_options', 'off' ) ;
    // On uninstall delety WPDA options.
    /**
     ...
     */
    const  OPTION_WPDA_DATATABLES_RESPONSIVE_VERSION = array( 'wpda_datatables_responsive_version', '2.2.9' ) ;
    const  OPTION_PLUGIN_NAVIGATION = array( 'wpda_plugin_navigation', 'dashboard' ) ;
    const  OPTION_PLUGIN_NAVIGATION_DEFAULT_PAGE = array( 'wpda_plugin_navigation_default_page', 'wpda' ) ;
     ...
     */
    const  OPTION_BE_TABLE_ACCESS = array( 'wpda_be_table_access', 'show' ) ;
    // Values: show, hide and select.
    /**
     * Option wpda_be_table_access_selected and it's default value
     */
    const  OPTION_BE_TABLE_ACCESS_SELECTED = array( 'wpda_be_table_access_selected', '' ) ;
    // Tables authorized in WPDA.
    /**
     * Option wpda_be_view_link and it's default value
     */
    const  OPTION_BE_VIEW_LINK = array( 'wpda_be_view_link', 'on' ) ;
    // Show view link in list table.
    /**
     * Option wpda_be_allow_insert and it's default value
     */
    const  OPTION_BE_ALLOW_INSERT = array( 'wpda_be_allow_insert', 'on' ) ;
    // Show insert link in list table (simple form).
    /**
     * Option wpda_be_allow_update and it's default value
     */
    const  OPTION_BE_ALLOW_UPDATE = array( 'wpda_be_allow_update', 'on' ) ;
    // Show update link in list table (simple form).
    /**
     * Option wpda_be_allow_delete and it's default value
     */
    const  OPTION_BE_ALLOW_DELETE = array( 'wpda_be_allow_delete', 'on' ) ;
    // Show delete link in list table.
    /**
     * Option wpda_be_wpda_export_rows and it's default value
     */
    const  OPTION_BE_EXPORT_ROWS = array( 'wpda_be_wpda_export_rows', 'on' ) ;
    // Show export link in list table (row export).
    /**
     * Option wpda_be_wpda_export_variable_prefix and it's default value
     */
    const  OPTION_BE_EXPORT_VARIABLE_PREFIX = array( 'wpda_be_wpda_export_variable_prefix', 'on' ) ;
    // Allows to import into repository with different wpdb prefix.
    /**
     * Option wpda_be_wpda_allow_imports and it's default value
     */
    const  OPTION_BE_ALLOW_IMPORTS = array( 'wpda_be_wpda_allow_imports', 'on' ) ;
    // Allow to import data (main only).
    /**
     * Option wpda_be_confirm_export and it's default value
     */
    const  OPTION_BE_CONFIRM_EXPORT = array( 'wpda_be_confirm_export', 'off' ) ;
    // Ask for confirmation before exporting.
    /**
     * Option wpda_be_confirm_view and it's default value
     */
    const  OPTION_BE_CONFIRM_VIEW = array( 'wpda_be_confirm_view', 'off' ) ;
    // Ask for confirmation before viewing.
    /**
     ...
     */
    const  OPTION_BE_DESIGN_MODE = array( 'wpda_be_design_mode', 'advanced' ) ;
    // Default design mode (basic/advanced).
    /**
     ...
     */
    const  OPTION_FE_TABLE_ACCESS = array( 'wpda_fe_table_access', 'select' ) ;
    // Values: hide, show, select.
    /**
     * Option wpda_fe_table_access_selected and it's default value
     */
    const  OPTION_FE_TABLE_ACCESS_SELECTED = array( 'wpda_fe_table_access_selected', '' ) ;
    // Tables authorized in WPDA.
    /**
     ...
     * @var array
     */
    protected static  $wpda_tables = array() ;
     ...
     * @var array
     */
    protected static  $wp_tables = array() ;
    protected static  $plugin_pages = array(
    ...
    {
        return 'wpda_wpdp_' === substr( $page, 0, 10 ) || WP_Data_Access_Admin::PAGE_EXPLORER === substr( $page, 0, 13 ) || in_array( $page, self::$plugin_pages );
    }
    ...
    public static function get_option( $option )
    {
        ...
        if ( isset( self::$option_cache[$option[0]] ) ) {
            return self::$option_cache[$option[0]];
            // Re-use cached value.
        }
    ...
    public static function is_wpda_table( $real_table_name )
    {
        ...
        global  $wpdb ;
        $phpdoc_supported_solution = self::WPDA_TABLES;
        // DO NOT DELETE THIS TO MAKE THE CODE SIMPLER!!! (read).
        return isset( $phpdoc_supported_solution[substr( $real_table_name, strlen( $wpdb->prefix ) )] ) && $wpdb->prefix === substr( $real_table_name, 0, strlen( $wpdb->prefix ) );
    }
    ...
    public static function set_option( $option, $value = null )
    {
        try {
            ...
            update_option( $option[0], $option_value );
            // Save option value in wp_options.
            self::$option_cache[$option[0]] = $option_value;
            // Save for re-use.
        } catch ( \Exception $e ) {
    ...
    public static function log( $log_id, $log_type, $log_msg )
    {
        global  $wpdb ;
        $sql = $wpdb->prepare(
            'INSERT INTO ' . $wpdb->prefix . 'wpda_logging (log_time, log_id, log_type, log_msg) VALUES (now(), %s, %s, %s)',
            $log_id,
            $log_type,
            $log_msg
        );
    ...
    )
    {
        ...
        foreach ( $columns as $column ) {
            ...
            if ( 'string' === WPDA::get_type( $column['data_type'] ) ) {
                $where_columns[] = $wpdb->prepare( "`" . str_replace( '`', '', $column['column_name'] ) . "` like '%s'", '%' . esc_sql( $search ) . '%' );
    ...
    public static function add_wpda_search_args( $columns )
    {
                ...
                if ( isset( $request["wpda_search_column_{$column_name_lwr}"] ) ) {
                    ...
                    if ( is_array( $request["wpda_search_column_{$column_name_lwr}"] ) ) {
                        ...
                        foreach ( $request["wpda_search_column_{$column_name_lwr}"] as $value ) {
                            ...
                            $column_value = wp_strip_all_tags( wp_unslash( $value ) );
                            // input var okay.
                            if ( '' !== $column_value ) {
                                
                                if ( 'string' === WPDA::get_type( $column_date_type ) ) {
                                    $where_columns_arr[] = $wpdb->prepare( "`{$column_name}` like '%s'", esc_sql( $column_value ) );
                                    // phpcs:ignore Standard.Category.SniffName.ErrorCode
                                } elseif ( 'number' === WPDA::get_type( $column_date_type ) ) {
                                    $where_columns_arr[] = $wpdb->prepare( "`{$column_name}` = '%d'", esc_sql( $column_value ) );
                                    // phpcs:ignore Standard.Category.SniffName.ErrorCode
                                }
                        ...
                        if ( '' !== $column_value ) {
                            ...
                            if ( 'string' === WPDA::get_type( $column_date_type ) ) {
                                $where_columns[] = $wpdb->prepare( "`{$column_name}` like '%s'", esc_sql( $column_value ) );
                                // phpcs:ignore Standard.Category.SniffName.ErrorCode
                            } elseif ( 'number' === WPDA::get_type( $column_date_type ) ) {
                                $where_columns[] = $wpdb->prepare( "`{$column_name}` = '%d'", esc_sql( $column_value ) );
                                // phpcs:ignore Standard.Category.SniffName.ErrorCode
                            }
        ...
        if ( 0 === count( $where_columns ) ) {
            ...
            $operator = ( isset( $_REQUEST['wpda_search_column_operator'] ) && 'or' === strtolower( $_REQUEST['wpda_search_column_operator'] ) ? 'or' : 'and' );
    ...
    public static function get_user_default_scheme()
    {
        ...
        if ( false !== $default_databases ) {
            ...
            if ( $user_id > 0 && isset( $default_databases[$user_id] ) && self::schema_exists( $default_databases[$user_id] ) ) {
    ...
    {
        global  $wpdb ;
        $rows = $wpdb->get_results( "select distinct post_type from {$wpdb->posts}", 'ARRAY_N' );
        $cpts = [];
        foreach ( $rows as $row ) {
            $cpts[] = $row[0];
        }
    ...
    public static function get_table_engine( $schema_name, $table_name )
    {
        ...
        return ( 1 === sizeof( $table_info ) && 'connect' === strtolower( $table_info[0]['engine'] ) ? 'connect' : '' );
    }
    
    public static function get_table_values( $schema_name, $table_name )
    {
        $wpdadb = WPDADB::get_db_connection( $schema_name );
        return $wpdadb->get_results( $wpdadb->prepare( "\n\t\t\t\t\t\tselect engine as engine,\n\t\t\t\t\t\t\t   table_rows as table_rows,\n\t\t\t\t\t\t\t   table_type as table_type\n\t\t\t\t\t\t from  information_schema.tables\n\t\t\t\t\t\twhere  table_schema = %s\n\t\t\t\t\t\t  and  table_name = %s\n\t\t\t\t\t", [ $wpdadb->dbname, $table_name ] ), 'ARRAY_A' );
    }
    ...
    public static function get_row_count_estimate( $schema_name, $table_name, $wpda_table_settings )
    {
        ...
        if ( 1 === sizeof( $table_info ) ) {
            ...
            if ( 'federated' === strtolower( $table_info[0]['engine'] ) || 'innodb' === strtolower( $table_info[0]['engine'] ) || stripos( strtolower( $table_info[0]['table_type'] ), 'view' ) !== false ) {
                // Handle InnoDB tables, views and system views
                $wpdadb = WPDADB::get_db_connection( $schema_name );
                $view_explain = $wpdadb->get_results( 'explain select count(*) from `' . str_replace( '`', '', $table_name ) . '`', 'ARRAY_A' );
                ...
                if ( is_array( $pk ) && sizeof( $pk ) > 0 ) {
                    $sql_rowcount_sql = 'select count(*) from `' . str_replace( '`', '', $table_name ) . '` where `' . $pk[0] . '` not in ' . '(select null from `' . str_replace( '`', '', $table_name ) . '` where 1=2)';
                    $wpdadb = WPDADB::get_db_connection( $schema_name );
    ...
    {
        if ( !ctype_alpha( substr( $item_name, 0, 1 ) ) || !preg_match( "/^[_a-zA-Z0-9]+\$/", $item_name ) ) {
            return false;
        }
        return true;
    }
    ...
    public static function get_sonce_token()
    {
        ...
        return base64_encode( openssl_encrypt(
            WPDA::get_option( WPDA::OPTION_PLUGIN_SONCE_SEED ) . $_SERVER['REMOTE_ADDR'],
            $encrypt_method,
            $key,
            0,
            $iv
    ...
    {
        return ( is_array( $input ) ? array_map( array( WPDA::class, 'sanitize_text_field_array' ), $input ) : (( is_scalar( $input ) ? sanitize_text_field( wp_unslash( $input ) ) : $input )) );
    }
