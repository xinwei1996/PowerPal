				...
				<div id="tabs">
					...
					<div id="tabs-2">
						...
						<div>
							...
							<span class="wpda_tooltip_sar" title="Search and replace:

(1) The SEARCH button initiates a case insensitive search
(2) The REPLACE and REPLACE ALL buttons perform a case sensitive replace">
...
EOL;
		...
		private function css() {
			...
			<style>
                ...
                #tabs-2 > div:last-child {
                    display: grid;
                    grid-template-columns: 65px auto 28px 110px;
                    align-items: center;
                }
		...
		private function js() {
			...
			<script>
				...
				function showTableOutput(schemaName, tableName, addReplaceButtons, searchString, replaceString) {
					let button = addReplaceButtons ?
						`<button type="button" class="button button-secondary replace-table-button" style="display:none" onclick="replace('${schemaName}', '${tableName}', '${searchString}', '${replaceString}')">Replace</button>` : '<span></span>';
				...
				function replace(schemaName, tableName, searchString, replaceString) {
					if (!replaceAllSelectedTables) {
						wpda_confirm("Replace?", `Replace all occurences of <strong>${searchString}</strong> with <strong>${replaceString}</strong> in table <strong>${tableName}</strong>?<br/><br/>This action cannot be undone!<br/>Are you sure you want to continue?`).then(function (a) {
							if (a) {
								replaceTable(schemaName, tableName, searchString, replaceString);
							}
						});
					} else {
						replaceTable(schemaName, tableName, searchString, replaceString);
				...
				function searchTable(schemaName, tableName, searchString) {
					jQuery.ajax({
						method: "POST",
				...
				function replaceTable(schemaName, tableName, searchString, replaceString) {
					jQuery.ajax({
						method: "POST",
				...
				function startReplace() {
					if (jQuery("#replace_searchbox").val().trim()==="" || jQuery("#replace_replacebox").val().trim()==="") {
						wpda_alert("Missing user input", "Please enter a search and a replace string.");
					}
				...
				function startSearch(addReplaceButtons = false) {
					if (jQuery("#searchbox").val()==="") {
						...
						if (jQuery("#searchbox").val().trim()==="") {
							wpda_alert("Oops!?", "Nothing to search. Are you trying to fool me...?");
						} else {
							...
							if (jQuery("#selectionDatabases .selectionFrameBody div input[type=checkbox]:checked").length>0) {
								...
								wpda_alert("No tables selected", "Please select one or more tables are try again.");
							}
							...
							jQuery("#selectionDatabases .selectionFrameBody div input[type=checkbox]:checked").each(function (i, elem) {
								...
								for (tableName in wpdaTables[schemaName]) {
									if (wpdaTables[schemaName][tableName]) {
										showTableOutput(schemaName, tableName, addReplaceButtons, searchString, replaceString)
										searchTable(schemaName, tableName, searchString);
									}
								}
							});
						}
		...
		public static function search() {
			...
			// Perform query.
			$result = $wpdadb->get_results( $query, 'ARRAY_N' );
			if ( '' === $wpdadb->last_error && is_array( $result ) && count( $result ) > 0 ) {
				WPDA::sent_header( 'application/json' );
		...
		public static function replace() {
			...
			if ( '' === $_POST['r'] ) {
				WPDA::sent_header( 'application/json' );
				WPDA::sent_msg( 'ERROR', 'Nothing to replace' );
				die();
			}
		...
		private static function check_request() {
			...
			if ( ! isset( $_POST['sn'], $_POST['tn'], $_POST['q'] ) || '' === $_POST['sn'] || '' === $_POST['tn'] ) {
				WPDA::sent_header( 'application/json' );
				WPDA::sent_msg( 'ERROR', 'Invalid arguments' );
				die();
			}
